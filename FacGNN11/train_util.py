...
def train(args,model,optimizer,with_eval):
    ...
    # train_check(output_dir = args.output_dir) # 检查是否有输出的结果的目录和文件
    ...
    if 0 == (forward_step % args.gradient_accumulation_steps): # 每一步都要
        torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm) # 梯度裁剪
        optimizer.step() # 启动优化器
        model.zero_grad() # 
        ...
        if 1 == update_step or 0 == update_step % args.evaluation_steps:
            ...
            with open(avg_loss_path_file, 'w', encoding='utf-8') as f:
                ...
                f.close()
            step_loss = {}
            logging.info("update step:[{}] ,save Train loss file[{}]".format(update_step, avg_loss_path_file))
